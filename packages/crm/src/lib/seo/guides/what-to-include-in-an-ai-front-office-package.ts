  ...
  sections: [
    {
      ...
      body: "A front office is everything that happens between a customer's first contact and a confirmed job. The phone gets answered. The text gets a reply. The web lead gets followed up before it goes cold, the appointment lands on the real calendar, and the review request goes out after the work is done.\n\nFor most small service businesses, that chain is currently held together by a voicemail box, a sticky note, and whoever's free. That's exactly why calls go unanswered and leads sit for hours.\n\nThe pitch for a packaged AI front office is that the buyer gets that whole chain **delivered and operated as one thing**, not five separate tools they have to stitch together and manage themselves. That's the difference between \"here's a chatbot\" and \"here's a front office\": one is a component, the other is an **outcome**.\n\nIf you're building the packaging method behind offers like this — bundling, naming, positioning a monthly service instead of a one-off build — that's covered in the companion piece on [productized AI services](/guides/productized-ai-services). This page is about what belongs inside the one flagship package, once you've decided to build it.",
      callout: {
        kind: "analogy",
        text: "A front office is a restaurant's front-of-house, not the kitchen. It's not the work itself — it's the host who seats you, the server who takes the order, and the check that gets settled at the end. Everything the customer actually touches.",
      },
    },
    {
      h2: "The component checklist",
      body: "**Phone agent** (24/7 or after-hours-only). This is usually the headline component. It's the easiest to justify too — every unanswered call is a lead going to the next business that picks up.\n\nMissed-call text-back. The moment a call is missed, an automatic text goes out. Even a business with no phone agent at all should have this — it's cheap to add once the phone agent exists.\n\nSpeed-to-lead follow-up on web forms. A form fill that sits for two hours is close to dead. A reply inside minutes is what separates a booked job from a lead that called three other businesses first.\n\nBooking into the real calendar, with *read-back* confirmation. The agent has to write to the calendar the business actually uses, then confirm the time back to the customer before treating the slot as booked. A booking nobody can verify is worse than no booking system at all.\n\nReview requests after the job is marked done. Timed to go out once, not nagged, tied to job completion rather than a fixed schedule.\n\nThe **CRM** where all of it lands. Every call, text, form, and booking needs one place a human can see it, or the \"front office\" is just a set of bots nobody can audit.\n\nThe **client portal** and the monthly proof report. This is what the client actually looks at to decide the retainer is worth it — call volume, leads captured, bookings made, reviews requested.\n\nOptional, add if the client wants it: website chat widget, appointment reminders. Explicitly out of scope for this package: running ads, and writing ongoing marketing content. Those are different disciplines with different pricing models — don't fold them in just because they're adjacent. Sell them separately if you offer them at all.",
      callout: {
        kind: "analogy",
        text: "A read-back is the front desk repeating your appointment time out loud before you hang up — a five-second check that catches a Tuesday booked as Thursday before it turns into a no-show.",
      },
      diagram: {
        type: "stack",
        title: "What's inside one AI front office workspace",
        layers: [
          { label: "Phone agent", sub: "24/7 or after-hours-only" },
          { label: "Missed-call text-back" },
          { label: "Speed-to-lead follow-up", sub: "web form leads" },
          { label: "Booking + read-back", sub: "real calendar, confirmed out loud" },
          { label: "Review requests", sub: "after job completion" },
          { label: "CRM", sub: "everything lands here" },
          { label: "Client portal + monthly report" },
        ],
      },
    },
    {
      h2: "The operating layer that justifies the retainer",
      body: "Anyone can hand a client a phone number and a bot. What justifies charging monthly — not just a one-time build fee — is the layer above the tools.\n\nThat layer is: **guardrails and escalation rules** written with the client (what the agent can promise, what it must never say, when it hands off to a human); a **weekly review** of the actual call and message logs in month one, while the agent is still being tuned to how this specific business talks to customers; and a monthly walk-through of the report with the client, rather than an email they might not open.\n\nBuying the tools is the cheap part. The client is paying for it still working next month, and the month after — when their hours change, a new service gets added, or the agent says something wrong on a call nobody caught. That **operating discipline**, not the phone agent itself, is what a retainer is actually for. It's the part a client can't easily replicate by buying software off a shelf.",
      callout: {
        kind: "tip",
        text: "Write the guardrails down with the client in week one, not after the first mistake. A rule you agree on calmly reads very differently from a rule you're writing in a panic right after an agent overpromises on a call.",
      },
    },
    {
      h2: "Onboarding week: staged, not big-bang",
      body: "Intake first: business hours, the service list, pricing policy (what the agent can quote and what it must defer), emergency criteria (what counts as urgent enough to interrupt someone), and tone — how this business actually talks to customers, not a generic script.\n\nConfigure the agent and calendar against that intake. Have the owner run test calls and correct anything that's off before a real customer ever reaches it.\n\nThen **launch in stages**: after-hours and overflow first, while the business's existing process still handles daytime calls. That way the first real customer interactions happen at low stakes, with a fallback already in place.\n\nOnly move to **full cutover** — the agent as the primary front line — once after-hours has run clean for a stretch. A staged launch means the first mistake gets caught by an owner reviewing a log, not by a customer who hung up.",
      diagram: {
        type: "flow",
        title: "Onboarding week, staged not big-bang",
        steps: [
          { label: "Intake", sub: "hours, services, pricing, tone" },
          { label: "Configure", sub: "agent + calendar against intake" },
          { label: "Owner test calls" },
          { label: "After-hours launch", sub: "daytime still human-run" },
          { label: "Full cutover", sub: "after a clean stretch" },
        ],
      },
    },
    {
      h2: "Pricing the package",
      body: "Anchor against what the client is replacing, not an invented market rate. A human answering service alone commonly runs a few hundred dollars a month before scheduling or review tools are added.\n\nRuby's published virtual-receptionist plans, for example, list $250/month for 50 minutes up to $1,725/month for 500 minutes (Ruby pricing, verified below) — and that's phone coverage only, with no CRM, booking, or review layer attached. A full front office replacing an answering service plus a scheduling tool plus a review tool is worth pricing against **that whole stack**, not against a single component.\n\nTier it rather than selling one flat price: after-hours-only as the entry tier, full front office (everything in the checklist above) as the core offer, and front office plus reputation management (review requests, response monitoring) as the top tier for clients who want more.\n\nStructure the price as a setup fee — covering intake, configuration, and the staged launch — plus a monthly retainer that covers the **operating layer**, not just the software. Keep the actual numbers hedged; what a market will bear varies by vertical, region, and what you're bundling. Run your own numbers through the margin calculator before quoting anything, rather than guessing at a rate.",
    },
    {
