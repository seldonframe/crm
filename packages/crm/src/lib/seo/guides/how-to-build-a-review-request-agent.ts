  ...
  sections: [
    {
      ...
      body: "Most agent pitches ask a client to trust a number you report — calls answered, leads qualified, hours saved. A review-request agent **doesn't need your report at all**.\n\nThe client opens Google Maps, searches their own business, and sees the review count and the date of the most recent one. That's the whole sales cycle, repeated every month.\n\nIt's provable because **review count and recency** are the two things buyers actually look at before they call. BrightLocal's 2026 Local Consumer Review Survey found that 97% of consumers read reviews for local businesses, and 85% say positive reviews make them more likely to use a business.\n\nAn agent that reliably turns finished jobs into review requests is directly working the exact signal a prospective customer checks first. That's why **it sells itself** once a client sees the before/after on their own profile.",
    },
    {
      h2: "The spec: what the agent actually has to do",
      body: "The trigger is a completed transaction, not a calendar date: invoice paid, booking marked complete, or deal moved to won.\n\nFire the review request **the moment a job wraps** — or a sane hour or two later, once the customer has had a moment to be happy rather than mid-task. That beats a generic weekly batch every time.\n\nThe message itself is one text or email with a direct review link (no search-and-scroll), sent to **every customer who completed the job** — not a filtered list.\n\nOne polite follow-up, a few days later, to anyone who hasn't clicked, and then stop. Repeated nagging burns the relationship for the sake of one more review.\n\nUnhappy replies need a separate lane entirely. If a customer responds with a complaint instead of clicking the link, that reply should **route straight to the owner**, not sit in an inbox waiting to become a public one-star review.\n\nTrack the two numbers that matter — requests sent vs. reviews landed — so the agent's output is a number, not a vibe.\n\nThe rule that isn't optional: Google explicitly prohibits \"discourag[ing] or prohibit[ing] negative reviews, or selectively solicit[ing] positive reviews from customers\" — commonly called *review gating* — and separately prohibits offering \"payment, discounts, free goods and/or services... in exchange for posting any review.\"\n\nA **compliant agent asks every customer** who completed a job, the same way, with no reward attached. The owner-routing step above exists so unhappy customers get handled directly by a human, not filtered out of the review ask itself — those are two different things, and conflating them is the compliance mistake that gets a Business Profile flagged.",
      callout: {
        kind: "warning",
        text: "Filtering the review request to happy customers only — even quietly, even \"just this once\" — is review gating. Google can suspend or flag the Business Profile for it, and there's no way to undo a review count once the pattern is visible. Ask everyone who completed a job, every time, no exceptions.",
      },
      diagram: {
        type: "flow",
        title: "The review-request agent's path, start to finish",
        steps: [
          { label: "Job marked complete" },
          { label: "Short delay", sub: "an hour or two" },
          { label: "Review request sent", sub: "to every completed job" },
          { label: "Unhappy replies rerouted", sub: "straight to the owner" },
          { label: "Review lands" },
        ],
      },
    },
...
Neither path is wrong. If you already run the CRM webhook plumbing for other automations, DIY is a reasonable afternoon. If you're standing up your fifth client this month, assembled is the one that doesn't eat your week.`,
    ...
    },
    {
      ...
      body: "The pitch writes itself because the proof is public. Pull up the prospect's Google profile next to a competitor's, and point at review count and how recent the last one is.\n\nA business with **40 reviews from the last three months** reads as active and trustworthy. One with 12 reviews and the newest one from a year ago reads as maybe-closed.\n\nThat gap is visible to the prospect without you saying a word.\n\nPrice it as a **small flat add-on**, not a standalone product. It bundles naturally onto a [review-response agent](/guides/how-to-respond-to-a-negative-review), a receptionist, or a booking agent you're already running for the same client, since it shares the same CRM connection and the same monthly conversation about what the agents did.\n\nResist quoting a specific review-count outcome. Promise the **process** — every completed job gets asked, unhappy ones get routed to the owner first — and let the client's own profile do the closing.",
    },
    {
      h2: "Operating it month to month",
      body: "Keep a short per-client tone template — plumbing and med-spa customers don't want the same text.\n\nKeep a **do-not-ask list** for anyone mid-dispute, mid-refund, or who's already complained. Asking those customers for a review is how a five-star pitch turns into a screenshot of a tone-deaf text.\n\nSend a short monthly number to the client: **requests sent, reviews landed**, and where the rating trend is heading.\n\nThat's the entire retention story for this agent — no dashboard required, just the same three numbers every month.",
    },
    {
      h2: "Failure modes to design against",
      body: "Review gating — filtering the ask to happy customers only — is the one that can get a Business Profile penalized, not just a bad look. Build the trigger to fire on every completed job, full stop.\n\nAsking at the wrong moment (mid-task, before the customer has actually experienced the result) gets ignored. Or worse, gets a review left in a neutral mood instead of a positive one.\n\n**Generic, obviously templated copy** (\"We'd love your feedback!\") gets skipped at a much higher rate than a message that names the actual job.\n\nAnd repeat customers getting asked after every single visit will eventually opt out of texts from the business entirely. The **suppression window should cover the relationship**, not just the last request.",
    },
  ],
  faq: [
    {
