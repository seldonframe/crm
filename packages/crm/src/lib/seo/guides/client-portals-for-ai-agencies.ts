  ...
  sections: [
    {
      ...
      body: "An agent that answers calls, captures leads, and books jobs perfectly in the background produces zero felt value if the owner never sees it happen. A month goes by. The invoice hits. The natural question is \"what am I actually paying for?\"\n\nThat's not because the agent underperformed. It's because its output was **invisible**. Cancellations in service businesses track *ambient* value, not delivered value — and an agent working silently delivers plenty of the second and none of the first.\n\nA client portal closes that gap. A screen where the owner can see calls answered, leads captured, and jobs booked turns invisible work into something they can point to. Done well, the portal is your monthly report — except it's self-serve and live all the time, instead of a PDF you send once a month.\n\nIt's the single highest-leverage piece of UI an agency can build. It's the difference between a client remembering why they signed up and a client wondering if they still need you.\n\nThis isn't a novel idea. It's an established pattern in service-business software generally. Field-service platforms like Jobber ship a customer-facing portal (a \"client hub\") specifically so the end customer can \"request work, approve quotes, review scheduled jobs, make payments\" without a phone call. Agency platforms like GoHighLevel sell branded client-facing portals as a paid add-on for the same reason: agencies want their own logo on the screen their clients use.\n\nAn AI agency's client portal is the same idea, aimed one level up the chain. Not the homeowner watching their own job — the business owner watching their agent work.",
      callout: {
        kind: "analogy",
        text: "A client portal is the security-camera feed for work that already happened offscreen — the job got done either way, but only one version leaves the owner with proof they can glance at.",
      },
    },
    {
      h2: "What belongs in a client portal (and what doesn't)",
      body: "Four things earn a place on the screen.\n\nThe **activity feed** — actual conversations the agent had, with its *read-backs* visible, so the owner can see the agent got the details right, not just that a conversation happened.\n\nThe **numbers that map to money** — leads captured, jobs booked, reviews collected. Those are the metrics an owner already thinks in, not agent uptime or token counts.\n\nTheir **calendar and contacts**, since that's the operational data they need day to day regardless of the agent.\n\nAnd **simple controls**: business hours, service list, the number to escalate to when something needs a human.\n\nAll four map to a question a client already has in their head.\n\nJust as important is what does *not* belong there. Model settings, prompt internals, system messages — none of it means anything to a business owner, and all of it invites second-guessing you don't want. Your **margin** — what you pay for the platform versus what you charge — is exactly the number that turns a value conversation into a price negotiation the moment it's visible. And other clients' anything: the fastest way to lose every client in one afternoon is a portal that leaks across accounts.\n\nThe rule of thumb: if a metric maps to a business outcome, show it. If it maps to how the agent works internally, or belongs to someone else's account, it stays out.",
      diagram: {
        type: "compare",
        title: "What goes on the screen",
        left: {
          heading: "Show the client",
          items: [
            "Activity feed with read-backs",
            "Leads, jobs booked, reviews collected",
            "Their calendar and contacts",
            "Hours, services, escalation number",
          ],
        },
        right: {
          heading: "Keep off the screen",
          items: [
            "Model settings and prompt internals",
            "Your margin (cost vs. what you charge)",
            "System messages and agent config",
    ...
    },
    {
      ...
      body: "The portal is your agency's front door, not a page with the platform's logo on it. That means a custom domain, your logo, your colors — and the underlying platform staying invisible to the client. A client who discovers which platform is running underneath your brand hasn't learned anything useful. They've just learned they could shop around you.\n\nThe non-negotiable underneath the branding is **per-client isolation**. Every query that resolves a client's data has to be scoped to that client's account, full stop. A portal that shows even one field from another client's workspace is the kind of leak that ends an agency relationship — and possibly the agency — regardless of how good the agent underneath it is.\n\nWhite-labeling is cosmetic. Isolation is structural. The structural piece is the one you can't cut corners on.",
    },
    {
      h2: "The transparency trade-off, honestly",
      body: "Showing a client the full conversation log is the single fastest way to build trust. They see exactly what their agent said, in their own customers' words, with nothing hidden.\n\nIt's also the fastest way to expose every mistake the agent makes. Every awkward answer, every missed detail, every hallucinated fact is sitting right there in plain view for the client to find.\n\nThe honest answer isn't to hide the logs. Hiding them just delays the moment a client finds out the hard way — it trades a manageable trust problem for an unmanageable one.\n\nThe answer is guardrails and a review workflow good enough that what's in the log is something you're willing to stand behind: *read-back* verification before a claim goes out, escalation for anything outside the agent's confidence, and a process for catching drift before the client does.\n\nAn agency that shows its logs is implicitly making a **never-lies promise** to that client. Make sure the agent can actually keep it before you turn the log view on.",
      callout: {
        kind: "analogy",
        text: "A read-back is the barista repeating your order back before making it — cheap, and it catches the wrong-drink problem before the milk is steamed.",
      },
    },
    {
