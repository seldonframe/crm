...
const steps = ["business", "clients", "process", "voice", "priorities", "review"] as const;

export function SoulWizard() {
  ...
  return (
    <section className="crm-card mx-auto w-full max-w-2xl p-6">
      <div className="mb-4">
        <p className="text-sm text-[hsl(var(--color-text-muted))]">Step {stepIndex + 1} of {steps.length}</p>
        <h1 className="text-2xl font-semibold">Soul Setup</h1>
      </div>
      ...
      <div className="mt-6 flex justify-between gap-3">
        <button type="button" className="h-10 rounded-md border px-4" onClick={goBack} disabled={stepIndex === 0 || pending}>
          Back
        </button>
